...
@pytest.fixture(params=[{"prose": "Test prose."}, {"prose": "Another test prose."}])
...
def test_load_json():
    data = {
        ...
        "mcq": [{"question": "q2", "options": ["o1", "o2"], "answer": "a2"}],
    }
...
def test_save_flashcards_as_csv(flashcard_data):
    ...
    with open("flashcards.csv", "r") as f:
        reader = csv.reader(f)
        assert list(reader) == [["Question", "Answer"]] + [
            [d["question"], d["answer"]] for d in flashcard_data
        ]
...
def test_save_mcq_as_csv(mcq_data):
    ...
    with open("mcq.csv", "r") as f:
        reader = csv.reader(f)
        assert list(reader) == [["Question", "Options", "Answer"]] + [
            [d["question"], ", ".join(d["options"]), d["answer"]] for d in mcq_data
        ]
...
@pytest.mark.parametrize(
    "prose,flashcards,mcq",
    [
        (
            "Test prose.",
            [{"question": "q1", "answer": "a1"}],
            [{"question": "q2", "options": ["o1", "o2"], "answer": "a2"}],
        ),
        (
            "Another prose.",
            [{"question": "q3", "answer": "a3"}],
            [{"question": "q4", "options": ["o3", "o4"], "answer": "a4"}],
        ),
    ],
)
